...
function HomePage({ user, onSignOut }) {
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axios.get(
        `/appointments/appointments/${user?._id || 'guest'}`
      );
  ...
  const handleBookAppointment = async () => {
    ...
    try {
      const response = await axios.post('/appointments/appointments', appointmentData);
      if (response.data.success) {
  ...
  const handleDeleteAppointment = async (id) => {
    try {
      await axios.delete(`/appointments/appointments/${id}`);
      fetchAppointments();
    } catch (error) {
